              ...
              <div class="push-body">
                  <div class="push-box">
                      <div v-if="allData[tag].select.length">
                          <Select class="sele" v-model="group_id" :placeholder="allData[tag].title">
                              <Option value="">{{allData[tag].title}}</Option>
                              <Option v-for="option in allData[tag].select" :key="option.id" :value="option.id" :title="option.name">{{option.name}}</Option>
                          </Select>
                      </div>
...
export default {
    ...
    methods: {
      /**
       * 显示modal框
       * @param {Object} 默认选中的数据，选中的用户ID数组和部门ID数组，如：{users: [1','2'], 'groups': ['1','2']}
       */
      show(data) {
          if(data) {
