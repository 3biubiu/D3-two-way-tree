  ...
  computed: {
    style () {
      if (this.atwho) {
        ...
        if (wrap) {
          let left = x + el.offsetLeft - el.scrollLeft;
            left  = (left+1 > 269 ? 269 : left) + "px";
            let addY = this.atwho.list.length * 27;
            addY = addY >= 135 ? 135 : addY;
            addY += 20;
          const top = y + addY + el.offsetTop - el.scrollTop + 'px'
          return { left, top }
        }
      }
      return null
