            ...
            <div class="ivu-transfer-list">
                ...
                <div class="ivu-transfer-list-body check-box-main">
                    <CheckboxGroup v-model="leftSelect" @on-change="(data)=>{checkAllGroupChange('left',data)}">
                        <Checkbox  class="check-item" v-for="(left,lk) in leftList" :label="left.id" :value="left.id" :key="left.id">&nbsp;&nbsp;{{left.name}}</Checkbox>
                    </CheckboxGroup>
                </div>
            </div>
            ...
            <div class="ivu-transfer-list">
                ...
                <div class="ivu-transfer-list-body check-box-main">
                    <CheckboxGroup v-model="rightSelect" @on-change="(data)=>{checkAllGroupChange('right',data)}">
                        <Checkbox :disabled="right.hasOwnProperty('disabled') && right.hasOwnProperty('disabled')==1" :title="(right.hasOwnProperty('disabled') && right.hasOwnProperty('disabled')==1)?'此载体为意向载体，如需删除请前往项目详情页【意向载体】中删除':''" class="check-item" v-for="(right,lk) in rightList" :label="right.id" :value="right.id" :key="right.id">&nbsp;&nbsp;{{right.name}}</Checkbox>
                    </CheckboxGroup>
                </div>
            </div>
        </div>
...
<script>
    /*
    * 2020-2-28 20:37:56
    * zyz于2020年开发并升级的穿梭框2.0升级版
    * 修复若干问题，使其支持的数据格式更为通用
    */
    export default {
        name: "CommonTransfer",
        props : {
                ...
                switch (type){
                    ...
                    case "right":
                        ...
                        if (this.rightSelectAll) {
                            // this.rightSelect = this.rightList.map((v)=>{
                            //     if (!(v.hasOwnProperty('disabled') && v.hasOwnProperty('disabled')==1)) {
                            //         return v.id
                            //     }
                            // });
                            this.rightSelect = [];
                            for(let v of this.rightList){
                                if (!(v.hasOwnProperty('disabled') && v.hasOwnProperty('disabled')==1)) {
                                    this.rightSelect.push(v.id);
                                }
                            }
                        } else {
                            this.rightSelect = [];
