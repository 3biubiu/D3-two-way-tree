    ...
    <card class="my-add-list" :padding="16">
        <div id="toway-tree" class="intellectual-property" xmlns="http://www.w3.org/1999/xhtml">
            ...
            <div class="operation-area">
                ...
                <tis-button type="default" @click="refreshZoom()">重置</tis-button>
            </div>
        </div>
        <div class="tooltip" v-show="showCard">
            悬浮卡片
        </div>
        <!-- <div>
...
export default {
    ...
    methods: {
        ...
        d3Click(selection) {
            ...
            selection.attr("cursor", 'pointer')
                ...
                .on('mouseout', function (d) {
                    ...
                    d3.selectAll("path[id =linkpath]").attr('stroke', '#d9d9d9').attr('stroke-width', 1)
                    d3.select(`rect[type = ${d.id}-hover]:not(.root-rect)`).attr("stroke", '#ECECEC').attr('stroke-width', 1)
                    d3.select(`rect[type = ${d.id}]:not(.root-rect)`).style("fill", '#FFF')
                    ...
                    //避免无效重复执行
                    // if (d3.select(this).node().tagName == "rect") {
                    // if(d3.select(this))
                    // console.log('d3.select(this): ', d3.select(this).node().tagName);
                    // }
        ...
        draw() {
            ...
            // 先绘制一遍源头公司名称计算dom宽度,因为源头矩形宽度自适应
            let textWidth = svg.append('text')
                ...
                }).style('font-size', '16px').style('font-family', 'PingFangSC-Medium').style('font-weight', function () {
                    return 'bold';
                })
            ...
            function update(source, showtype, sourceTree) {
                ...
                nodeEnter.append('rect')
                    ...
                    .attr("class", function (d) {
                        // if(!d.depth) {
                        return !d.depth ? `root-rect` : ''

                    })
                    .attr('width', function (d) {
                        return d.depth ? _this.diamonds.w : _this.originDiamonds.w;
                    }).attr('height', function (d) {
                        return d.depth ? (_this.diamonds.h) : _this.originDiamonds.h;
                    }).attr('x', function (d) {
                ...
                nodeEnter.append('text')
                    ...
                    .attr('fill', function (d) {
                        ...
                        return d.depth ? "14px" : "16px"
                    }).style('font-family', 'PingFangSC-Medium').style('font-weight', function (d) {
                        return d.depth ? 'normal' : 'bold';
                    })

                // 名称过长 第二段
                nodeEnter.append('text', function (d) {
                ...
                clickNode.append('circle').attr('type', function (d) {
                    ...
                    .attr("fill", "#fff").attr("storke-width", 1).attr('stroke', "#1F6CDD").attr("cursor", "pointer");
                ...
                clickNode.append('svg:line')
                    ...
                    .attr("stroke", function (d) { return d.depth ? '#1F6CDD' : "#1f6cdd" })

                    .attr('stroke-width', '1').attr('x1', '0').attr('y1', function (d) {
                        return - 3.125;
                    })
                ...
                clickNode.append('svg:line')
                    ...
                    .attr("class", 'horizontal-line').attr("stroke", '#1F6CDD').attr('stroke-width', '1').attr('y1', 0)
                    .attr('x1', '-3.125').attr('y2', 0)
                    .attr('x2', '3.125').attr('type', function (d) {
                        return d.id || (d.id = showtype + 'text' + ++_this.i);
                    })
                    .attr("cursor", "pointer")
                ...
                nodeEnter.append('marker')
                    ...
                    .attr('orient', '90')
                    .attr('refX', function () {
                        return showtype === 'up' ? '-56' : '62';
                    })
                    .attr('stroke', 'none') // 防止箭头继承路径的属性 iebug
                    .append('path').attr('d', 'M0,0L0,-5L8,0L0,5')
                    .attr('fill', '#1f6cdd').attr('fill-opacity', 1);
                ...
                let nodeUpdate = node.transition().duration(function (d) {
                    if (d.data.children != null && d.data.children.length > 10) {
                        ...
                        return _this.transitionTime - 200
                    }
                })
                    .attr('transform', function (d) {
                        // console.log('移动节点');
                ...
                let linkEnter = link.enter().insert('path', 'g')
                    ...
                    // marker-start  marker-end 一个在markstart绘制 一个在end 绘制
                    // 根据箭头标记的id号标记箭头
                ...
                //绘制线条
                linkUpdate.transition().duration(_this.transitionTime - 300).attr('d', function (d) {
                    return diagonal(d.parent, d, showtype);
                });

                link.exit().transition().duration(_this.transitionTime - 300).attr('d', function (d) {
                    let o = {
                        x: source.x0,
                        y: source.y0
                    };
